...
public class SessionUtil extends HttpServlet implements Serializable {
	...
	public void RemoveAllSession(HttpServletRequest request) {
		...
		String[] keyArr = CmeProperties.getProperty("Secure.Input.Name").split(",");
	 ...
	 * @작성자 : Kim, YunKwan
	 * @변경이력 :
	 * @Method 설명 : object 형태로 세션값을 리턴합니다. 키값이 없으면 null - 리턴받은후 알맞은 Type으로 변경 [ex.
	 *         (String)GetSessionValue(reuqest,name) ]
	 * @param request
	 * @param name
	 * @return
	 */
	...
	public String getPreMenuId() {
		...
		while (names.hasMoreElements()) {
			String key = (String) names.nextElement();
			/*
			 * if (key.equals(SiasSessionConst.CurMenuId)) { return (String) session
