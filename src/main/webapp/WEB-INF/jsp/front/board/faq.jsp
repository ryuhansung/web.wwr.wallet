...
<script>
    $(document).ready(function(){
        ...
        $('.swiper-wrapper a').each(function() {
            ...
            if('${vo.cat_id}' != "" && activeTab == '${vo.cat_id}'){
    ...
    function goSearch(){
        ...
        $("#pageIndex").val("1");
        loadFrm('', 'faqForm', 'post', '/front.board.list.dp/proc.go');
    }

    function optionCange(catId) {
        $("#cat_id").val(catId);
        $("#pageIndex").val("1");
        $("#searchWrd").val('');
        loadFrm('', 'faqForm', 'post', '/front.board.list.dp/proc.go');
    ...
    <div class="contents-wrap">
        <div class="tgroup">
            ...
            <ul>
                ...
                <li><spring:message code="TEXT_14"/></li>
            </ul>
        </div>
        <div class="searchTab-wrap">
            <form id="faqForm" name="faqFrom" action="POST">
            <div class="swiper-container slide_mainMenu">
                <ul class="swiper-wrapper" style="height: 40px">
                    <li class="swiper-slide"><a href="#n" data-tab="" class="active" ><spring:message code="TEXT_170"/></a></li>
                    <c:if test="${!empty optList}">
                        <c:forEach items="${optList}" var="optList">
                            <li class="swiper-slide"><a href="#n" data-tab="${optList.cat_id}">${optList.cat_name}</a></li>
                        </c:forEach>
                    </c:if>
                </ul>
            </div>
            <div id="tab1" class="search-container ui tab active">
                <div class="search-wrap">
                    <label><spring:message code="TEXT_14"/></label>
